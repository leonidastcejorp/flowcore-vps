#!/usr/bin/env python3
"""📋 ErrLog — ngumpulin semua error dari monitor & ngasih tau lo."""
import json
import os
import sys
...
try:
    ...
    CACHE_FILE = os.path.expanduser("~/.hermes/cron/output/error_summary_report.txt")
    os.makedirs(os.path.dirname(CACHE_FILE), exist_ok=True)
...
def main(all_flag=False, clear_flag=False):
    if clear_flag:
        ...
        print("✅ Error log dibersihkan (yang >14 hari dihapus)")
        return

    entries = ErrorLog.get_recent(200)
    if not entries:
        _simpan_cache(None)
        print("✅ **ErrLog:** Bersih — semua monitor jalan normal.")
        return
    ...
    lines = []
    lines.append(f"{header_icon} 📋 ErrLog")
    lines.append("")
